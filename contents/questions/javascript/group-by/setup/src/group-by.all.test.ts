...
import groupByGFE from './group-by';
import groupByAlt from './group-by-alt';

describe('groupBy', () => {
  [groupByLodash as typeof groupByGFE, groupByGFE, groupByAlt].forEach((groupBy) => {
    test('empty array', () => {
    ...
    describe('property iteratees', () => {
      test('single-element arrays', () => {
        expect(groupBy(['one'], (o: string) => o.length)).toEqual({
          3: ['one'],
        });
      });
      ...
      test('multiple element arrays', () => {
        expect(
          groupBy(['one', 'two', 'three'], (o: string) => o.length),
        ).toEqual({
          3: ['one', 'two'],
          5: ['three'],
        });
