...
柯里化在实际开发中并不常用，但对于面试来说，这是一个比较常见的问题，因为它考察了候选人对某些 JavaScript 基础知识的理解，例如元数和闭包。
...
我们可以通过闭包来记录到目前为止的柯里化函数参数。每次调用柯里化函数时，我们将到目前为止的参数数量与原始函数的元数进行比较。

* 如果它们相同，我们使用这些参数调用原始函数。
* 如果需要更多参数，我们将返回一个接受更多参数的函数，并使用新参数调用柯里化函数。

此问题的解决方案也适用于 [Curry](/questions/javascript/curry/)，因为此解决方案是一个通用版本，它不对所需参数的数量做任何假设。
...
由于最内层的函数本质上是为了保留 `this` 作用域并传递参数，因此可以使用 `Function.prototype.bind` 来实现：
...
* 访问 `this` 的函数。 也要测试这种柯里化函数的情况，这些函数也应该接收多个参数。
