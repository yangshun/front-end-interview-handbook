...
- `lastTickTiming`: This is the time that the last interval callback has run. We will keep incrementing the `totalDuration` by the delta between the current time (`Date.now()`) and the `lastTickTiming`. Using this approach, the `totalDuration` will still be accurate even if the callbacks run at irregular intervals. Since it's not used in the render code, we can store it as a local variable instead of Vue's reactive state.
- `isRunning`: A [computed property](https://vuejs.org/guide/essentials/computed.html) that checks if the timer is currently running. This is determined by whether `timerId` is null or not.
