...
When managing state in a modern front-end framework, it is considered best practice to avoid direct mutation of data. Therefore, we make a copy of the `order` array first before calling `pop()` on it to remove the last item.
