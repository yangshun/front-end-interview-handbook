...
We can use `setInterval` with a duration of 300ms and remove the last value of the `order` array by using `order.pop()` each time the interval callback is invoked. When the `order` array is empty, we can clear the interval timer.
