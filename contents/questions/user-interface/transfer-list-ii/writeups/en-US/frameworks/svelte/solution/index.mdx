...
The following explanation assumes you have a good understanding of [Transfer List's Svelte solution](/questions/user-interface/transfer-list/svelte/solution).
...
Did you know that an HTML checkbox actually has 3 states? They are checked, unchecked, and indeterminate. Indeterminate state is not commonly used and cannot be set via HTML attributes. Luckily, Svelte provides a way to set the indeterminate state via the `indeterminate` attribute in template.
