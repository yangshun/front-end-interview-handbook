...
* `alt`: 图像的文本描述，用于图像加载失败时和屏幕阅读器。
* `fetchpriority`: 提供获取图像时使用的相对优先级的提示。Pinterest 使用 `fetchpriority="auto"`
* `loading`: 指示浏览器应如何加载图像。Pinterest 使用 `loading="auto"`，这是默认值。可能的值包括 `"lazy"`，它将加载推迟到图像或 iframe 达到与视口的距离阈值。
* `src`: 图像 URL。
...
* **非常宽的图像**：非常宽的图像将具有非常小的高度，因为它们必须在列宽内保持纵横比。因此，用户可能只能看到一条细的水平线，并且几乎不可见。当一个 1000px 宽和 20px 高的图像放入 100px 宽的列中时，在保持纵横比的同时，其高度将只有 2px。
