...
* 将每个标准放在查询参数中会使查询字符串变得非常长，并且会丢失层次结构。
* 查询参数名称与非便利设施字段发生冲突的可能性。这可以通过使用`amenities_`作为所有便利设施字段的前缀来解决（例如`amenities_breakfast`和`amenities_washer`）。
* 此外，某些便利设施标准具有非原始值，无论如何都需要进行 URL 编码和解码。
...
* **图片预加载/懒加载**：一种非常有用的技术是使用 JavaScript 来精细控制图像的加载时间。
  * [Airbnb 通过结合懒加载和预加载在其房间列表中优化了图片轮播体验](https://medium.com/airbnb-engineering/building-a-faster-web-experience-with-the-posttask-scheduler-276b83454e91)行为：
    1. 最初，仅加载第一张图片（其余图片将进行懒加载）。
    2. 当用户表现出查看更多图片的可能意图时，会预加载第二张图片：
       * 光标悬停在图片轮播上。
       * 通过制表符聚焦于“下一步”按钮。
       * 图片轮播进入视图（在移动设备上）。
    3. 如果用户确实查看了第二张图片（这表明有浏览更多图片的强烈意图），则会预加载接下来的三张图片（第 3 到 5 张）。
    4. 当用户再次单击“下一步”以浏览更多图片时，会预加载第 (n + 3)<sup>th</sup> 张图片。
...
* **响应式图片**：为设备提供最合适的图片。
  * [网络上的图片：第 1 部分 — 响应式图片 | Expedia Group Technology](https://medium.com/expedia-group-tech/images-on-the-web-part-1-responsive-images-5dc0066461bd)
  * [网络上的图片：第 2 部分 — 实现响应式图片 | Expedia Group Technology](https://medium.com/expedia-group-tech/images-on-web-part-2-implementing-responsive-images-ca1d30f533f8)
...
* 地图
  * 彼此靠近的标记可以[聚集在一起](https://developers.google.com/maps/documentation/javascript/marker-clustering)
* 白标
