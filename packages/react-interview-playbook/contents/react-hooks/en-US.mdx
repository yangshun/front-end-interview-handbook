...
> [!TIP] Default to the functional setter whenever the next state depends on the previous one
>
> `setCount(count + 1)` reads `count` from the closure at the time the handler was created, which breaks inside intervals, async callbacks, and rapid successive updates. `setCount((prev) => prev + 1)` always reads the latest committed state.
...
React compares state by reference to decide whether to re-render, so mutating the object in place is invisible to React.

> [!WARNING] Never mutate state objects or arrays in place
>
> Assigning to `user.age` or calling `array.push(...)` changes the contents but not the reference, so React will not re-render. Always produce a new object or array via spread, `map`, `filter`, or a setter callback.
...
> [!IMPORTANT] If you reach for useEffect in an interview, pause and ask whether you need it
>
> Most interview questions are self-contained with no external system to sync to. State derived from other state, logic that runs in response to a user event, or data that could be computed during render should not live inside an effect.
...
Omitting a dependency is one of the most common `useEffect` bugs in interviews and in production code.

> [!WARNING] Every reactive value the effect reads must appear in the dependency array
>
> Omitting a prop or state value to silence a re-run silently captures a stale version of it. The effect will keep seeing the initial value forever, leading to mysterious bugs that only surface when the omitted value changes.
...
>
> React identifies each hook by its call position, not its name. Conditionally calling a hook or calling hooks inside a loop causes the indices to shift between renders, silently corrupting state across all hooks that follow.
...
> [!NOTE] The "use" prefix is how the linter and React know a function is a hook
>
> Without the prefix, the rules-of-hooks linter cannot enforce call-order invariants. If your helper never calls another hook, keep it a plain function rather than inventing a hook name.
