...
> [!IMPORTANT] Good state design is often what separates strong interview solutions from weak ones
>
> Before you start coding, spend a minute identifying the minimal set of values the UI actually needs. Interviewers frequently probe this; the rest of the implementation flows naturally from a clean state shape.
...
> [!TIP] If a value can be computed from existing state or props, derive it during render
>
> Storing derivable values creates two sources of truth that must be kept in sync, almost always via a `useEffect`. Plain variables recomputed on each render are cheaper than you'd expect and eliminate a class of staleness bugs.
...
> [!TIP] Changing a component's `key` is the cleanest way to fully reset its internal state
>
> This unmounts and remounts the subtree, clearing every `useState`, ref, and effect-owned resource. It's especially handy when a component has lots of internal state that would be tedious to reset field by field.
...
> [!WARNING] Every consumer re-renders on any provider value change
>
> Context is not a performance primitive. Putting frequently-changing data (e.g. text input values, mouse position) into a single provider re-renders the entire subtree on every keystroke. Split contexts by update frequency, or reach for a proper state library.
