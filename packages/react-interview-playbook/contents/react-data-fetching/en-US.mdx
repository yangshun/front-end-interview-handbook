...
1. Basics of data fetching:
   - Using the `fetch` API and `axios` for API requests
...
3. Optimizing data fetching:
   - Implementing debouncing and throttling to prevent redundant API calls
   - Using pagination and infinite scroll for large datasets
   - Avoiding duplicate requests by caching previous queries
4. Handling data mutations (POST, PUT, DELETE):
   - Using optimistic updates vs. re-fetching after mutations
   - Rolling back state when mutations fail
   - Invalidating queries after a mutation
5. (For take-home assignments) Using TanStack Query:
   - Automatically caches and re-fetches data
   - Reduces unnecessary state management with `useState` and `useEffect`
   - Handles background refetching and pagination

## Key interview questions on data fetching
