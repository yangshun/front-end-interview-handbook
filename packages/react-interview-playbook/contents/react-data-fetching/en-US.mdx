...
> [!WARNING] Debouncing alone does not fix race conditions
>
> If a slow response outlives a newer query, the stale response can still overwrite fresher data. Always pair debouncing with either an `AbortController` or an "ignore" flag in the effect cleanup to discard outdated responses.
...
If the component unmounts before a request finishes, its response is no longer useful. React 18 and later no longer warn about state updates after unmounting, but Effects should still clean up requests to prevent stale responses and stop unnecessary work. One approach is to abort the request with an `AbortController` in the Effect's cleanup function.
...
> [!TIP] Mention a query library when asked to scale up client-side data fetching
>
> For production or take-home contexts, name-dropping TanStack Query or SWR and explaining what you'd get for free (caching, deduping, refetching, cancellation) shows awareness of what the hand-rolled `useEffect` approach is missing.
