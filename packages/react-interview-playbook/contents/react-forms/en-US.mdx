...
> [!TIP] Default to controlled inputs in interviews unless the form is trivial
>
> Interviewers frequently tack on follow-ups like live validation, conditional fields, or programmatic reset. Those are straightforward with controlled state but require extra plumbing with refs and `FormData`.
...
> [!NOTE] Checkboxes bind to `checked`, not `value`, and read `event.target.checked`
>
> This is a common interview slip. Text inputs use `value` and `event.target.value`; checkboxes and radios require the boolean `checked` attribute and read the boolean off `event.target.checked`.
...
> [!TIP] Reach for `useReducer` only when several fields update together or depend on each other
>
> For independent fields, `useState` plus a generic `handleChange` keyed on `event.target.name` is usually simpler. Reducers shine when a single action should touch multiple fields atomically (e.g. reset, step transitions, conditional defaults).
...
function DynamicForm() {
  ...
  function addField() {
    ...
    nextId.current += 1;
    setFields((currentFields) => [...currentFields, { id, value: '' }]);
  }
...
> [!WARNING] Client-side validation is only a UX convenience, never a security boundary
>
> Attackers can skip your React form entirely and hit the endpoint directly. Every constraint you care about – required fields, length, format, uniqueness – must be re-checked on the server.
