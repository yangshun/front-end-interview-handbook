...
React handles most events through delegation: native listeners are attached to the root of the React application rather than the document root, and React dispatches each event to the appropriate component handlers.
...
Mouse event handlers fire when users interact with the mouse (clicks, hovers, etc.). Mouse events can be added to most elements, but an element that acts as an interactive control should use a semantic element such as `<button>`, `<a>`, or `<input>` instead of a generic `<div>` or `<span>`. Semantic controls provide the expected keyboard and screen reader behavior.
...
For accessibility purposes, do not make a non-interactive element behave like a control solely by adding an `onClick` handler.

> [!WARNING] Use semantic elements for interactive controls
>
> A bare `<div>` or `<span>` does not become keyboard-focusable or gain button semantics when an `onClick` handler is added. If it looks and behaves like a button, it should be a `<button>`. An ancestor container may still listen for bubbled events for purposes such as event delegation or analytics, provided the actual interactive control remains semantic.
...
> [!NOTE] React's `onChange` is not the DOM's `change` event
>
> React reimplements `onChange` to fire on every keystroke, aligning it with the DOM's `input` event rather than the browser's `change` (which fires on blur/commit). This is one of React's most-quizzed quirks.
...
> [!TIP] Always call `event.preventDefault()` in SPA form submit handlers
>
> Forgetting this causes a full-page reload and loses any client-side state. Put it as the first line of every `handleSubmit` until it becomes muscle memory.
...
However, there are cases where this behavior is undesirable, such as when a button inside a toolbar should handle its own action without also triggering the toolbar's ancestor handler.

```jsx
function Toolbar() {
  function handleToolbarClick() {
    console.log('Click bubbled to the toolbar!');
  }
...
The button remains the semantic interactive control; the surrounding `<div>` only demonstrates how a descendant click bubbles to an ancestor listener. In interviews, there shouldn't be many instances where you need to nest `onClick` handlers. If you ever do so, consider if you need `event.stopPropagation()` in the inner event handler.
