...
> [!WARNING] Never attach `onClick` to a bare `<div>` or `<span>`
>
> Non-interactive elements are not keyboard-focusable and not announced as clickable by screen readers. If it looks like a button, it should be a `<button>`. Interviewers grading for accessibility will catch this immediately.
