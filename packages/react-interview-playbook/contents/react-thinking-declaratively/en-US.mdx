...
2. We can use a `Set` for `completedTasks` and store only the task ID. This way, the task titles aren't repeated and looking up a task's completion status is fast. However, we still need to remember to modify two places when a complete task is being deleted.

Instead of storing multiple independent pieces of state that require syncing, here's a minimal and structured state representation:
...
Given a UI and the requirements, you should be able to think in a declarative fashion to define the various necessary components, props, state, operations that change the state, and connecting all of them together.
