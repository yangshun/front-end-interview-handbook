...
> [!IMPORTANT] In React, you describe the UI for the current state — you never reach into the DOM to mutate it
>
> If you find yourself wanting to call `document.querySelector` or manually toggle classes, that is a sign the data should be state instead. Let the render function be the single source of truth for what's on screen.
...
> [!TIP] Design the state shape before writing any JSX
>
> Walking through UI states, possible actions, and the minimal data needed makes the component fall out of the design almost mechanically. Interviewers often listen for this reasoning more than the final code.
