...
> [!IMPORTANT] Every form input needs an associated `<label>`
>
> Placeholders are not labels — they disappear when the user types and screen readers may skip them. Use `<label htmlFor="...">` paired with an `id`, or wrap the input inside the `<label>`.
...
> [!WARNING] HTML5 validation is a UX hint, not a security control
>
> Attackers bypass the browser entirely by calling your endpoint directly. Anything you care about – required fields, length, format – must also be enforced server-side.
