...
> [!TIP] Default to function components with hooks for every interview question
>
> Unless the interviewer explicitly asks for a class component, function components are the modern, expected style. Attempting a class version just adds lifecycle boilerplate you don't need.
...
> [!IMPORTANT] Props flow down, state stays local — this is React's one-way data flow
>
> Children cannot mutate props, and parents cannot reach into a child's state. To share state across siblings, lift it up to the nearest common parent and pass it back down as props.
