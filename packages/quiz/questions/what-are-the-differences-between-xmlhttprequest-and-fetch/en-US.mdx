...
However, `fetch` provides a unified response object with methods for accessing data using the then method.
...
You can abort a `fetch` but it requires a [AbortController](https://developer.mozilla.org/en-US/docs/Web/API/AbortController) object:
...
However, the `fetch` API does not offer any way to track upload progress.

### Choosing Between XHR and Fetch
