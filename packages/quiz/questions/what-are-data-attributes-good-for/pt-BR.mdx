...
Outro caso comum de uso dos atributos `data-` é armazenar informações usadas por bibliotecas ou frameworks de terceiros. Por exemplo, a biblioteca Bootstrap usa atributos de dados para causar `<button>`s executar ações em um modal em outro lugar na página ([example]\(https://getbootstrap. om/docs/5.2/components/modal/#via-data-attributes)).
