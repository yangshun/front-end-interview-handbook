...
只包括必要的最小的 CSS/内容/脚本，以便在用户浏览器中首先呈现尽可能快的页面量，然后你可以使用延迟脚本或监听`DOMContentLoaded`/`load`事件来加载其他资源和内容。

## 异步 HTML 片段

在后端构建页面时，将 HTML 的部分内容刷新到浏览器。 关于这一技术的更多详情可查阅 [这里](http://www.ebaytechblog.com/2014/12/08/async-fragments-reinding-progressive-html-rendering-with-marko/)。

## 其他现代技术

- [渐进式水合](https://www.patterns.dev/posts/progressive-hydration/)
- [流式 SSR](https://www.patterns.dev/posts/server-side-rendering/)
