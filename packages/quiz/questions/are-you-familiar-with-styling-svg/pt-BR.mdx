...
O `fill="purple"` acima é um exemplo de um _atributo apresentacional_. Curiosamente, e ao contrário de estilos embutidos como `style="fill: purple"` que também é um atributo, os atributos de apresentação podem ser [sobrescritos pelo CSS]\(https://css-tricks. estilos de om/presentation-attributes-vs-inline-styles/) definidos em uma folha de estilos. Daí se você fez algo como `svg { fill: blue; }` ele irá substituir o preenchimento roxo que foi definido.
