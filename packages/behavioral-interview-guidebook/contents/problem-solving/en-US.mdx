...
There are simply too many types of problem solving and drive for results behavioral questions that could be asked, such as:
...
> [!TIP] Pick a story where you both identified and measured the problem
>
> Candidates frequently lose points by describing only the solution. Interviewers look for the full loop — how you surfaced the real problem, how you defined success, and how you verified it worked — so a story that omits those phases caps how high it can score.
...
> [!NOTE] Prefer stories where you initiated the problem, not just solved it
>
> "Manager told me to fix X" rates lower than "I noticed X was broken and scoped the fix." Interviewers weight proactivity heavily — if your best solution story was assigned rather than surfaced, flag any initiative you showed inside it (e.g. defining metrics, expanding scope).

## Sample story

Here's a sample story using the STAR format.

**Situation**: I was the tech lead for an e-commerce website selling luxury goods. The website was built as an Angular.js 1.6 single-page application. In recent years, the product was showing its age – developer experience was not great and the website performance was poor. Initial loading speed was over 3 seconds and the conversion rate was around 0.8%.
...
3. **Solution brainstorming**
   ...
   - **Performance-centric mindset**: Read many performance case studies on [web.dev](https://web.dev/) and engineering blogs of other e-commerce companies, gathered a list of important performance techniques and processes:
     ...
     - Consolidated duplicate JavaScript libraries (date-fns and moment.js), switched to `lodash-es`, removed all usages of jQuery
...
> [!IMPORTANT] Tailor depth to the question — do not recite the whole project
>
> A full project log is not an answer. Cover every phase briefly, but only zoom in on the sub-plot the question is actually probing (e.g. tradeoffs, data use, creativity). Reciting everything crowds out the signal and eats the interviewer's follow-up time.
