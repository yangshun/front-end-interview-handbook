...
这个问题受到 JavaScript API `Intl.ListFormat.prototype.format()` 函数的启发。
...
* `length` 选项将数组分成两部分。
* 第一部分将用 `, ` 连接，第二部分将用 `and` 连接。 - 是否显示 `X other(s)` 取决于 `length` 值是否在 `[0, items.length]` 范围内。

<MDXCodeBlock languages={{ jsx: listFormatJs, tsx: listFormatTs }} />

## 笔记

此函数不如 `Intl.ListFormat.prototype.format()` API 可用，因为分隔符被硬编码为英语。 `Intl` API 旨在用于国际化 (i18n)，并且还允许自定义分隔符（逗号和 `and`），因此如果此函数用于生产环境，则不应硬编码任何分隔符！
