...
Implement a `deepClone` function that performs a deep clone operation on JavaScript objects. You can assume the input object only contains JSON-serializable values (`null`, `boolean`, `number`, `string`, arrays, objects) and will not contain any other objects like `Date`, `Regex`, `Map` or `Set`.
