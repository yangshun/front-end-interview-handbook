...
Now I have a utility function `shouldDeepCompare` that will tell us whether we should traverse down the current property of the input object based on the data type retrieved by `getType`. When the current property is _not_ an object or an array, we can proceed to compare their types and values via `Object.is`.
