...
import compact from '../src/compact-ii';
import compactFunctional from '../src/compact-ii-functional';

## Solution

Since the input value can be deeply nested, a recursive solution will be handy for traversing the nested values. The idea is to recursively visit all values in the input, filtering out the falsey values and keeping only the non-falsey ones.
