import MDXCodeBlock from 'MDXCodeBlock';

import countBy from '../setup/src/count-by';
import countByAlt from '../setup/src/count-by-alt';

## Solution

1. Create an empty `results` object to store the count of occurrences of each key.
1. Determine the `iteratee` function. If `iteratee` is already a function, we can use it as-is. Otherwise, if a string value was provided, a function is created to access that property on an element.
1. Iterate through the array and determine the key for each element by calling `iteratee(element)`. If the key does not exist within the `results` object, set the value for that key to 0. Next we can increment the value for that key.

<MDXCodeBlock>{countBy}</MDXCodeBlock>

An alternative way to increment the `result` counter is to use the [nullish coalescing assignment operator](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Nullish_coalescing_assignment) to set the value to 0 if `key` doesn't exist within `result`. Note that using nullish coalescing assignment operator means you might be accessing inherited properties, which is not desired, but since the object is created via `Object.create(null)`, there will not be inherited properties and is safe to use.

<MDXCodeBlock>{countByAlt}</MDXCodeBlock>

## Resources

- [Lodash `_.countBy`](https://lodash.com/docs/#countBy)
