...
excerpt: Implement a class that can subscribe to and emit events which triggers attached callback functions. Susbcription objects are returned and can unsubscribe itself
---
...
Returns an object that has an `off()` method that unsubscribes this listener from the event. More details below.
...
This object is returned from an `emitter.on()` call. Calling `sub.off()` unsubscribes the respective listener from the event.
