...
import solution from '../setup/src/event-emitter-ii';
...
Implementing `EventEmitter.on()` is pretty straightforward. Firstly check if `eventName` is present as a key of the `_events` object and make the value an empty object (for the map of `listenerId`s to its listeners for that event) if it is the first time this `eventName` is encountered. Then push the `listener` into the array.
