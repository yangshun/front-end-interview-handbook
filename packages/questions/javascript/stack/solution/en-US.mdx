...
import solution from '../src/stack';
import linkedListSolution from '../src/linkedListSolution';
...
`push()`-ing involves creating a new `Node` and pointing the current top of the stack to that new `Node`'s `prev` field and updating a reference to the new top of the stack. `pop()`-ing is the reverse where we find the `prev` node of the current top and set it to be the new stop of the stack.
