...
import arrayAt from '../src/array-at';
import arrayAtSpec from '../src/array-at-spec';
...
Note that the specifications state that the `index` is converted to an integer first, so the function works even for string versions of indices.

<MDXCodeBlock>{arrayAt}</MDXCodeBlock>
