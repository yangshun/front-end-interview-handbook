---
title: Drop While
excerpt: Implement a function that excludes elements from the beginning of an array until the predicate returns false
---

Implement a function `dropWhile(array, predicate)` that creates a slice of `array` excluding elements dropped from the beginning. Elements are dropped until `predicate` returns falsey. The predicate is invoked with three arguments: _(value, index, array)_. Your function should not modify the original array.

#### Arguments

1. `array` _(Array)_: The array to query.
1. `predicate` _(Function)_: The function invoked per iteration.

#### Returns

_(Array)_: Returns the slice of `array`.

## Examples

```js
dropWhile([1, 2, 3, 4, 5], (value) => value < 3); // => [3, 4, 5]
dropWhile([1, 2, 3], (value) => value < 6); // => []
```

Note that Lodash's `dropWhile` utility also allows you to pass an optional `thisArg` parameter to bind `this` inside the predicate function, but for this exercise, you can ignore that parameter.

## Resources

- [Lodash `_.dropWhile`](https://lodash.com/docs/#dropWhile)
