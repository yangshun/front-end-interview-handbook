...
Note that the inner function needs to be defined using arrow functions to preserve the same lexical `this` or manually tracked using a separate variable like in [Debounce](/questions/javascript/debounce).
...
- Functions which access `this`. Do test this case for curried functions receiving multiple arguments as well.
