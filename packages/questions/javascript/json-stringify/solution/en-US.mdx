...
import jsonStringifySwitch from '../src/json-stringify-switch';
...
- `number`s: Directly convert into a string via `String()`.
- `string`s: We have to wrap the value with double quotes as it's required by JSON.
...
This question is a simplified version of `JSON.stringify` and doesn't handle many other types available in JavaScript nor the formatting options. Some other cases:
