...
* `boolean`：通过 `String()` 直接将 `true`/`false` 转换为字符串。
* `number`：通过 `String()` 直接转换为字符串。
* `string`：我们必须用双引号将该值括起来，因为 JSON 需要它。
* 数组：递归地将每个子项字符串化，然后用逗号连接它们，并将它们括在方括号中：`[` 和 `]`。
* 对象：通过递归地将值字符串化，将每个键/值对（也称为条目）转换为 `"key":{stringifiedValue}` 格式，用逗号连接它们，并将它们括在大括号中：`{` 和 `}`。
