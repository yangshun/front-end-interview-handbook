...
import functionApply from '../src/function-apply';
import functionApplyAlt from '../src/function-apply-alt';
import functionApplyCall from '../src/function-apply-call';
import functionApplySymbol from '../src/function-apply-symbol';
...
As `Function.prototype.apply()` takes in an array of arguments, we have to define a default empty array parameter for `args` if we are to spread it, because we can only spread iterables.
...
<MDXCodeBlock>{functionApply}</MDXCodeBlock>
