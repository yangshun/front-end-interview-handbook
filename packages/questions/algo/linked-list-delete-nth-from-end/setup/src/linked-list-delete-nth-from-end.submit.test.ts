import deleteNthNodeFromEndOfList from './linked-list-delete-nth-from-end';
...
describe('deleteNthNodeFromEnd', () => {
  (submitTestCases as any[]).forEach((example: any) => {
    test(`numbers = [${example.input[0][1]}] n = ${example.input[1][1]}`, () => {
      ...
      const result = linkedListToVector(
        deleteNthNodeFromEndOfList(head, example.input[1][1]),
      );
