...
- **Space complexity: O(m.n)**. The result array `result` stores all elements of the matrix in spiral order.
...
- **Time complexity: O(m.n)**. Each element in the matrix is visited exactly once.
- **Space complexity: O(1)**. The traversal is performed in place, and no additional space is required apart from the result array.
