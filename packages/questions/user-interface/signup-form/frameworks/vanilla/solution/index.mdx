...
There are a [more validation attributes](https://developer.mozilla.org/en-US/docs/Web/Guide/HTML/Constraint_validation) but the above are the ones relevant to signup forms.

All the fields are `required`, so we can add this attribute to all the `<input>`.
