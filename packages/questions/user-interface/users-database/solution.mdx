...
As user is selected, the first and last name fields are updated with the selected user's data. Theese fields are empty if there is no selected user.

We use simple buttons for creating / updating / deleting / cancaling actions:
