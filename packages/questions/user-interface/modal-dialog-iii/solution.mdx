...
We can create a generic hook called `useOnKeyDown` that takes in a string key representing the keyboard key to respond to, and a callback to trigger when the key is pressed.
...
We need to obtain a reference to the modal dialog body. In React this is achieved using the `useRef()` hook.
...
To use the hook, call it with the following arguments: `useOnClickOutside(dialogRef, onClose)`.
