...
We need a timer to refresh the page and display the current time every second. To create the timer, we can use a `setInterval` from Web API and kick off the update loop in a `ngOnInit`.

Do remember to clear the timer in `ngOnDestroy` to avoid errors and memory leaks.
