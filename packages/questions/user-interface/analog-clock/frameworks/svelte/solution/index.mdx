...
We need a timer to refresh the page and display the current time every second. To create the timer, we can use a `setInterval` and kick off the update loop in an `onMount`.

Do remember to clear the timer upon unmounting of the component to prevent memory leaks.
