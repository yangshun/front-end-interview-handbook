...
We'll build on top of [Tabs' solution](/questions/user-interface/tabs/angular/solution). Other than adding the right ARIA roles and states, which is straightforward, we also need to link the tab items with the corresponding tabpanels. Hence we create two functions, `getTabListItemId` and `getTabPanelId` to do this.
