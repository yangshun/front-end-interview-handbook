...
We'll build on top of [Accordion's solution](/questions/user-interface/accordion/vue/solution).
...
Since there can be multiple `Accordion` component instances on the page and we cannot guarantee that the accordion section values will be globally unique, each `Accordion` instance needs to have a unique identifier. We can use a simple technique `Math.random().toString(36).substring(2, 9)` to generate unique ID for each `Accordion` instance. The final ID string will be a concatenation of the `Accordion` instance's ID, the item value, and whether it's a header or a panel.
