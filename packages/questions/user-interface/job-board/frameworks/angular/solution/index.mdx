...
Firstly, in `ngOnInit()` hook, we will utilize the `HttpClient` service to initially load all job ids, save them to a property
named `allJobIds` and subsequently call the `fetchJobs()` function to retrieve the first job contents.

Within the `fetchJobs()` function, we will determine the job ids to be loaded, using the `PAGE_SIZE` constant so
every time we call `fetchJobs()` we will load the next job contents and if ids property will be empty it won't be
executed in `forkJoin`.

We will initiate requests equivalent to the number indicated by `ids.length` and using `forkJoin`, an array of the
loaded jobs will be returned. Subsequently, we will update the `jobs` with the newly loaded ones using the spread operator.
