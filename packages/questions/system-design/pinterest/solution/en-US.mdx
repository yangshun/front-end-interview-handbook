...
To achieve responsive images, an alternative approach is to use the `<picture>` tag. The differences between `<img srcset>` or `<picture>` are subtle and for most cases both can do the job. The article ["Picture Tags vs Img Tags. Their Uses and Misuses"](https://medium.com/@truszko1/picture-tags-vs-img-tags-their-uses-and-misuses-4b4a7881a8e1) does a decent job covering the differences.

#### Image preloading

The images for the initial load are preloaded via `<link rel="preload">`:
...
There are multiple image formats to use, each with their own pros and cons. The general recommendation these days is use the [WebP format by Google](https://developers.google.com/speed/webp) if browser compatibility is not the highest priority. Pinterest's audience is the average consumer and hence browser support is crucial. Pinterest uses the JPEG format for images likely due to JPEGs having wider browser support (WebP isn't supported on IE11 and older versions of Safari) and because progressive JPEGs already provide a great loading experience.
...
However, one huge downside is that the DOM order is now column-first. Imagine a keyboard user has loaded some pins and wants to get to the top pin in the rightmost column. With such a DOM:
...
The keyboard user has to tab through two columns of pins before finally reaching that item. This is counter-intuitive when the desired pin is right at the top. This layout approach results in a really inefficient DOM order for keyboard users, and is a deal-breaker.
...
With absolute positioning, the DOM order of the items does not affect the visual result at all. Hence we are free to arrange the DOM such that the DOM order is a better reflection of the tabbing order.

The following example is implemented using the absolute positioning approach and the numbers within the items indicate the elements' tabbing order. Notice that there's only one layer of children nodes within the container.
...
- Multiple layout strategies are supported:
  ...
  - **Uniform row**: Yields a grid of tiems in rows of uniform height. The height of the row will be the tallest item in the row. this is the same as a normal table behavior. ([source](https://github.com/pinterest/gestalt/blob/master/packages/gestalt/src/Masonry/uniformRowLayout.js))
