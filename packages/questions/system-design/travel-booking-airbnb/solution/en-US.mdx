...
- **Localized**: The users are from many different countries and from various age groups. To capture a larger market, the website should be translated and localized.
- **Wide Device Support**: The site should work well for a variety of devices since the user demographics is very broad.

The requirements and nature of a travel booking website are very similar to the [e-commerce website question](/questions/system-design/e-commerce-amazon), so there is a high degree of overlap between the solutions. So in this solution we will cover the unique aspects of travel booking websites that aren't found on e-commerce websites.
...
As long SSR is being used, whether SPA or MPA doesn't matter as much. Both are viable, as long as the website has optimized performance.
...
Let's take a look at travel booking sites in the wild and their rendering choices:
...
**All the top travel booking sites in the world use SSR and React!** This shows the importance of SSR for travel sites and the dominance of React in the industry.

Many of these sites intentionally open the listing details in a new tab/window so as to preserve convenient access to the search results page. As a result, the website doesn't benefit as much from being a single page app as the most commonly navigated pages cannot reuse the app shell and client state. A MPA architecture can be used.

The discussion below assumes that we'll be using an MPA architecture that uses server-side rendering + hydration due to the need for interactivity after initial load.
...
To recap, the most important aspects of a travel booking website are: **Performance**, **Search Engine Optimization**, **Localization**, and **Wide Device Support**.
...
1. Other sites like travel blogs to link to the results page of specific search filters (e.g. vacations rentals in San Francisco). This helps to improve the site's SEO and discoverability.

#### Generate pages for popular searches
