...
> [!IMPORTANT] Design component APIs around multiple independent instances
>
> Whether you use the jQuery-style root element pattern or React's props model, your API must allow the same component to be instantiated many times on a page with independent state. Global variables, singletons, and hardcoded DOM IDs all break this and are common interview red flags.
...
If the loading order of the stylesheet is not guaranteed (e.g. if stylesheets are lazily loaded), the visual result will not be deterministic. The result is that developers start using hacks like `!important` or `.my-custom-slider.my-custom-slider` to let their selectors win the specificity war. With all these hacks, the CSS code starts becoming unmaintainable.

> [!WARNING] Class injection leads to non-deterministic styling
>
> When library and consumer selectors share specificity, the winner depends on stylesheet load order. Call this out in interviews — it is a classic trap that surfaces when stylesheets are code-split or lazily loaded, and it is why `!important` proliferates in real codebases.
...
> [!NOTE] Inline styles duplicate per instance and bloat the DOM
>
> A theme object rendered as inline styles repeats every declared property on every rendered component. For hot components rendered hundreds or thousands of times, this adds up in both payload and memory — something to raise in system design discussions.

The theme object is just a way to restrict the styling to certain properties and optionally, an accepted set of values, the values do not need to be used as inline styles and instead can be combined with other styling approaches.
...
[CSS variables](https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_custom_properties) (or more formally known as CSS custom properties) are entities defined by CSS authors that contain specific values to be reused throughout a document. The `var()` function accepts fallback values if the given variable is not set.
...
> [!TIP] Prefer logical properties like `margin-inline-start` over `margin-left`
>
> Logical properties adapt automatically to RTL, vertical, and other writing modes. Mentioning them in a component API design interview is a cheap way to signal i18n awareness without writing custom direction-handling code.
