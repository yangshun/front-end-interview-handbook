...
> [!TIP] Prioritize trees and traversals over exotic algorithm topics
>
> Front end interviewers rarely ask hard dynamic programming or graph problems. Spending your limited prep time on trees, recursion, BFS, and DFS pays off more directly because the DOM itself is a tree and many UI coding problems reduce to tree traversal.
...
Before diving into the tips, note that most of them are about communication habits, not algorithmic tricks. Interviewers reward candidates who surface tradeoffs and edge cases rather than those who silently produce code.

> [!TIP] Ask to use missing data structures instead of building them
>
> JavaScript lacks native queues, heaps, and binary search. Ask the interviewer if you can assume they exist and use them directly. This keeps the focus on the core algorithm and avoids burning interview time reimplementing a priority queue.
