...
> [!IMPORTANT] Keep state minimal and derive the rest
>
> Every extra state field is another opportunity for values to drift out of sync. Identify the smallest set of essential state and compute everything else from it on the fly — this pattern alone eliminates a huge class of UI bugs.
...
[React's](https://react.dev/) docs on ["Managing State"](https://react.dev/learn/managing-state) is an excellent resource on how to design and use component state correctly. Some of the ideas mentioned aren't specific to React and can be applied to any UI framework.
...
- **Heading hierarchy**: Ensure heading tags have a hierarchy and there's no more than one `<h1>` in the DOM.
...
> [!WARNING] Attaching click handlers to `<div>` or `<span>` breaks accessibility
>
> Non-semantic interactive elements are invisible to keyboards and screen readers unless you manually add `role`, `tabindex`, and key handling. Use `<button>` or `<a>` instead — interviewers treat clickable divs as a serious red flag.

### Forms

Forms are complex on their own and deserve their own section.
...
> [!WARNING] Parallel network requests can finish out of order
>
> Autocomplete, search, and filter components are especially prone to this — a stale response can overwrite fresh results. Track the latest request ID and discard earlier responses, or serialize requests by disabling the input while one is in flight.
...
- **Screen readers, ARIA roles, states, and properties**:
  - Add appropriate ARIA roles to custom elements that are not built using native HTML elements.
  - Use `aria-label`s to describe elements where text is not shown (e.g. icon-only buttons).
  - Link error message elements with the elements responsible for them via `aria-describedby`/`aria-errormessage`.
...
> [!WARNING] Never render untrusted input with `innerHTML` or `dangerouslySetInnerHTML`
>
> User-supplied HTML is the most common XSS vector. Use `textContent` or React's default escaping unless you have explicitly sanitized the input with a library like DOMPurify.
