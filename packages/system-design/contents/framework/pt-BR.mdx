...
Agora precisamos pensar nos campos de dados presentes no cliente. Os dados em aplicativos cliente geralmente têm uma de duas origens:
...
### Dados originados no cliente

Dados originados no cliente são criados ou modificados no cliente. Alguns precisam ser enviados ao servidor para persistência, enquanto outros só precisam existir temporariamente no cliente como estado da interface.

Os dados originados no cliente podem ser subdivididos em dois:
...
Ao listar os campos de dados, é útil identificar o tipo de cada campo e se ele é originado no servidor ou no cliente. Para dados originados no cliente, indique também se precisam ser persistidos ou se podem permanecer efêmeros.
