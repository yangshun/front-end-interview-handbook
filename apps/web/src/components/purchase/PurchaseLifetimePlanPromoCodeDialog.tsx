...
export default function PurchaseLifetimePlanPromoCodeDialog({
  ...
  // If the discounted price is 0, we use stripe as the payment provider
  // because tazapay doesn't support 0 amount.
  const paymentProvider = discountedPrice === 0 ? 'stripe' : 'tazapay';
    ...
    <Dialog
      ...
      primaryButton={
        <Button
          ...
          size="md"
          variant="primary"
          onClick={() => onContinue(promoCodeData?.id, paymentProvider)}
        />
      }
      secondaryButton={
        <Button
