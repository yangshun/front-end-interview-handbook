This challenge requires knowledge of form submission, using `fetch` for client-to-server communication, and positioning elements that are out of the normal page flow. If you've done the [Newsletter Section](https://www.greatfrontend.com/projects/challenges/newsletter-section) challenge, you'll find this one a breeze as it is largely similar, except with more fields.

## Relevant concepts

1. **Form handling and client-side validation**: Use HTML [`<form>`s](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form) to structure your contact subscription form for collecting user input. JavaScript can be used to handle [the `submit` event](https://developer.mozilla.org/en-US/docs/Web/API/HTMLFormElement/submit_event), prevent default behavior, and perform client-side validation to ensure that the user input meets the required criteria (e.g., valid email format). Techniques like [regular expressions](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_expressions) or built-in browser validation APIs ([`checkValidity()`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLSelectElement/checkValidity), [`setCustomValidity()`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLSelectElement/setCustomValidity), etc.) can help with validations.
...
Start by structuring the contact subscription section within the overall page layout using [semantic HTML elements](https://developer.mozilla.org/en-US/docs/Glossary/Semantics#semantics_in_html) and CSS. Utilise responsive design principles, such as CSS Flexbox or Grid, to create a layout that adapts gracefully to different screen sizes and devices. Test the responsiveness of the layout across various viewports to ensure a consistent user experience.
...
### Making email and telephone number links

You can make email addresses and telephone numbers clickable by using the `mailto:` and `tel:` protocols, respectively. This way, when the user clicks on the email address or telephone number, their email client or dialer app will open with the email address or telephone number pre-filled. `mailto:` and `tel:` are some of the many [protocols](https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/manifest.json/protocol_handlers#protocol) defined in the web standards.

```html
<a href="mailto:hello@abstractly.com">hello@abstractly.com</a>
<a href="tel:16505550198">+1 (650) 555-0198</a>
```

### Building the real-time character counter

You can use [the `input` event](https://developer.mozilla.org/en-US/docs/Web/API/Element/input_event) to listen for changes in the `<textarea>` as the user types, and display the character count with `event.target.value.length`. You can change the indicator colour to red when the character count exceeds the maximum length.

Remember to validate this before you make the POST request! While you can use [the `maxlength` attribute](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/maxlength) to limit the message length, an adversay can easily bypass this by removing the attribute in their browser's web inspector.

### Implement client-side form validation

Before making the POST request to the [message API](https://www.greatfrontend.com/projects/challenges/contact-section/assets), you should validate the user's input, i.e., email address and message character count. You can do this in the method you define for handling the form submission, i.e., [the `submit` event](https://developer.mozilla.org/en-US/docs/Web/API/HTMLFormElement/submit_event) of the `<form>` element.
...
Upon successful client-side validation, use [`fetch`](https://developer.mozilla.org/en-US/docs/Web/API/fetch) to send the form data to the server asynchronously. Construct an [HTTP POST request](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/POST) to the [message API endpoint](https://www.greatfrontend.com/projects/challenges/contact-section/assets), passing the user's email address as the request body as JSON or FormData. Handle the server's response, which may include a success message or an error message, and update the UI accordingly.
