...
Upon clicking on "Get in touch" button or "customer support" links, the page should scroll smoothly to the contact section and focus on the "Name" `<input>`. There are many ways to achieve this with JavaScript, but the easiest is using [`scrollIntoView`](https://developer.mozilla.org/en-US/docs/Web/API/Element/scrollIntoView) with [`behavior: 'smooth'`](https://developer.mozilla.org/en-US/docs/Web/API/Element/scrollIntoView#behavior) on the enclosing `<form>`. You can set a [`scroll-margin`](https://www.google.com/search?q=scroll+margin&oq=scroll+margin&sourceid=chrome&ie=UTF-8) on the `<form>` to control how far from the top will it be scrolled to.
