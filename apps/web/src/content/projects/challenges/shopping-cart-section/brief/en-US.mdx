...
- **Structure**: The page is divided into two main columns
  - The left column for cart items
  - The right column for the order summary
- **Empty state**: If there are no items in the cart, we should display the empty state as per the supplied design.

#### Cart items (Left column)

- **Default sort**: Latest item added to cart first
- **Item denomination**: Each product variant that has been added to cart should be displayed separately instead of bundled into 1 product. For e.g., if I added both Orange and Pink StepSoft socks, they should display as separate cards in cart.
...
- **"Add coupon code" field**:
  ...
  - **On clicking 'x' on the applied coupons**: cRemove the coupon from the order summary
