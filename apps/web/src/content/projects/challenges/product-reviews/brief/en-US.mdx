...
This interactive component will display user reviews for a specific product including a summary of the ratings. It will also allow users to filter reviews by rating, and have a pagination component when there are too many reviews on a page.
...
- **Default sort**: Latest review first
- **Default filter**: None
- **Pagination**:
...
- **Profile picture**: If present, display image. Else, display letters of up to first 2 initials.
- **Rating stars**: Display filled stars according to the rating given.
- **Review**: Display review if given. User is allowed to give rating without review e.g. Darren Holmes for Voyager Hoodie product.
...
- **Filtering by rating bands**:
  - Users can click on a rating bar to filter the review list by the rating. Users should return to the first page of reviews if they were not on the first page.
  - Implement the rating bar review states - Normal, Hover and Filter active.
  - One rating bar selection can be active at any one time.
