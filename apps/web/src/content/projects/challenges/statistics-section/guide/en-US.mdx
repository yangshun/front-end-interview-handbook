...
3. **Implement the metric list**: Fetch statistics from the API and render them. If you are using a templating system or a JavaScript framework, render each item using iteration helpers rather then repeating the metric markup.
4. **Display the image**: Render the image while making sure it has the same height as the right column on desktop and fills the entire 6-column section width. For breakpoints below desktop, the image has a fixed height.
