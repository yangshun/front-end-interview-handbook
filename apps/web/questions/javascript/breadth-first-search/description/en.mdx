...
1. Initialize a queue to store nodes to be visited. Push root node.
...
1. Enter a loop that continues until the queue is empty. In each iteration of the loop:
   1. Pop the top node from the queue and mark it as visited.
   1. Retrieve the neighbors of the node from the input graph.
   1. Enqueue the neighbors of the current node.
1. Return the set of visited nodes.

BFS is useful for the same purposes as [DFS](/questions/javascript/depth-first-search), and it is especially useful for finding the shortest path between two nodes.
