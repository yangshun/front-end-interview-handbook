...
import squashObject from '../src/squash-object';
...
<MDXCodeBlock>{squashObject}</MDXCodeBlock>
...
function chunk(array, size = 2) {
  // This is a helper function that groups two adjacent items in an array into one subarray.
  const chunkedArray = [];
  while (array.length) {
...
export default function squashObject(object) {
  ...
  return Object.fromEntries(chunk(squashObject(object)));
}
```
