...
We'll build on top of [Accordion's solution](/questions/user-interface/accordion/react/solution). Other than adding the right ARIA roles and states, which is straightforward, we also need to link the accordion headers with the corresponding accordion contents/panels. Hence we create two functions, `getAccordionHeaderId` and `getAccordionPanelId` to do this.
